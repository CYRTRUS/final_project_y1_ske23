...
class StatsScene(BaseScene):
    PAGE_TITLES = [
        ...
        "Damage Dealt (every 20s boxplot)",
        "Damage Received (scatter)",
        "Time to Complete Level (bar graph)",
    ]
    NUM_PAGES = 6

    def __init__(self, screen, switch_scene_callback, click_sound, game=None):
        ...
        self.buttons = [
            ...
            Button(
                675, 736, 120, 44, "Prev",
                self._prev_page, click_sound, color=(60, 80, 140)
            ),
            Button(
                805, 736, 120, 44, "Next",
                self._next_page, click_sound, color=(60, 80, 140)
            ),
        ]
        self._data = []
        self._refresh()
    ...
    def _p0_clicks(self):
        ...
        self._draw_axes(area, "Time (20s intervals)", "Clicks", x_ticks, y_ticks, rotate_x=True, tick_font=self.small_tick_font)

        n = max(len(keys) - 1, 1)
        pts = [(ax + int(i / n * aw), ay + ah - int(c / max_c * ah)) for i, c in enumerate(counts)]
        if len(pts) >= 2:
    ...
    def _p2_histogram(self):
        ...
        # Draw axes without x labels - we'll place them manually at bar centers
        self._draw_axes(area, "Damage dealt", "Frequency", [], y_lbl, tick_font=self.small_tick_font)

        for i, c in enumerate(counts):
            bx = ax + i * (aw // bins) + 1
            bh = int(c / max_c * ah)
            pygame.draw.rect(self.screen, (200, 80, 80), (bx, ay + ah - bh, bar_w, bh))
            # x label centered under each bar
            lbl_str = str(int(mn + i * bw_val))
            lbl = self.small_tick_font.render(lbl_str, True, (200, 200, 200))
            cx_bar = bx + bar_w // 2
            self.screen.blit(lbl, lbl.get_rect(centerx=cx_bar, top=ay + ah + 5))

    # Page 3: Damage dealt box-plot (20s buckets)

    def _p3_boxplot(self):
        triples = _gather_pairs(self._data, "damage_dealt", 20)
        bkts = _bucket_by_interval(triples, 20)
        if not bkts:
            self._no_data()
            return
        ...
        n = len(keys)
        bw = min(60, aw // max(n, 1) - 10)

        # Labels show the range for each bucket (e.g. "0-19", "20-39")
        x_ticks = [f"{k * 20}-{k * 20 + 19}" for k in keys]
        y_ticks = list(range(0, max_v + 2, max(1, (max_v + 1) // 5)))

        self._draw_axes(area, "Time (20s intervals)", "Damage dealt", x_ticks, y_ticks, rotate_x=True, tick_font=self.small_tick_font)
        ...
        for i, k in enumerate(keys):
            ...
            lo, hi = vs[0], vs[-1]
            # Center each box within its equal section to avoid y-axis overlap
            px = ax + int((i + 0.5) / n * aw)

            def ty(v):
                return ay + ah - int(v / max_v * ah)

            pygame.draw.line(self.screen, (200, 200, 200), (px, ty(lo)), (px, ty(hi)), 2)
            pygame.draw.rect(self.screen, (80, 160, 220), (px - bw // 2, ty(q3), bw, ty(q1) - ty(q3)))
            pygame.draw.line(self.screen, (255, 200, 80), (px - bw // 2, ty(q2)), (px + bw // 2, ty(q2)), 3)
    ...
    def _p4_scatter(self):
        ...
        self._draw_axes(area, "Time (20s intervals)", "Damage received", x_ticks, y_ticks)

        # Offset all dots right by one bucket width so the first dot is off the y-axis
        bucket_w = aw // n_buckets
        for rt, d in rel_pairs:
            px = ax + bucket_w + int(rt / max_t * (aw - bucket_w))
    ...
    def _p5_level_time(self):
        ...
        for session in sessions:
            ...
            sorted_lvls = sorted(lvl_first.keys(), key=lambda x: int(x) if x.isdigit() else 0)
        ...
        self._draw_axes(area, "Time category", "Number of levels", [], y_ticks)
        ...
        for i, (label, val) in enumerate(zip(labels, values)):
            ...
            self.screen.blit(cat_lbl, cat_lbl.get_rect(centerx=bx + bar_w // 2, top=ay + ah + 8))

    def _no_data(self):
        msg = self.font.render("No data yet - play a game first!", True, (200, 200, 200))
        self.screen.blit(msg, msg.get_rect(centerx=self.width // 2, centery=self.height // 2))
